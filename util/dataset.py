...
class SemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, base_data_root=None, data_list=None, data_set=None,
        ...
        fss_list_root = './lists_bam/lists/{}/fss_list/{}/'.format(data_set, mode)
    ...
    def __getitem__(self, index):
        ...
        image_path, label_path = image_path[3:], label_path[3:]
        image = cv2.imread(os.path.join("D:/PFENet_try/PFENet/", image_path), cv2.IMREAD_COLOR)

        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        image = np.float32(image)
        label = cv2.imread(os.path.join("D:/PFENet_try/PFENet/",label_path), cv2.IMREAD_GRAYSCALE)
        # label_b = cv2.imread(os.path.join(self.base_path,label_path.split('/')[-1]), cv2.IMREAD_GRAYSCALE)
        label_b = cv2.imread(os.path.join("D:/PFENet_try/PFENet/",label_path), cv2.IMREAD_GRAYSCALE)
        ...
        for k in range(self.shot):
            ...
            support_image_path, support_label_path = support_image_path[3:], support_label_path[3:]
            support_image = cv2.imread(os.path.join("D:/PFENet_try/PFENet/",support_image_path), cv2.IMREAD_COLOR)
            support_image = cv2.cvtColor(support_image, cv2.COLOR_BGR2RGB)
            support_image = np.float32(support_image)
            support_label = cv2.imread(os.path.join("D:/PFENet_try/PFENet/",support_label_path), cv2.IMREAD_GRAYSCALE)
        ...
        if self.mode == 'train':
            ...
            return image, label, label_b, s_x, s_y, subcls_list, support_image_path, image_path, class_chosen
